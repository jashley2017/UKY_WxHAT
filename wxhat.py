...
def write_sample(channel):
    new_row = [ # TODO: this is irresponsible but clever, dictionaries are not usually ordered, but the return values for these functions statically order them
            ...
            *thp.get_humidity().values(),
            ...
            BNO080_I2C.parse_rep(imu_recent_reports.get(BNO080_I2C.SENSOR_REPORTID_ROT, [[IMU_ERR]])[-1]), # the last in the list is the most recent
            BNO080_I2C.parse_rep(imu_recent_reports.get(BNO080_I2C.SENSOR_REPORTID_ACC, [[IMU_ERR]])[-1])
...
def run_hat():
    ...
    # start ADS as thread
    ads_proc = Process(target=ads.get_rtd_temp_cont, args=(ads_code,))
    ads_proc.start()

    
    # start GPS as a thread
    gps_proc = gps_startup()
